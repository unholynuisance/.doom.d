...
#+name: current-system-p
#+begin_src emacs-lisp :eval :session
(defun satan/current-system-p (&rest args)
  (pcase system-type
        ('gnu/linux
         (string-match-p (concat "^ID=\\(?:" (mapconcat 'identity args "\\|") "\\)")
                         (org-file-contents "/etc/os-release")))
        ('windows-nt
         (member "Windows" args))
        (default nil)))
#+end_src

As [[current-system-p]] is not evaluated during tangle, we need to somehow enforce evaluation.
