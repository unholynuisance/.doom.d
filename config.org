...
*** Systemd daemon
Since =2.6= emacs has a systemd serice file, so lets throw a =.desktop= file for =emacsclient=.
...
For some reason when I open new emacsclient frame it comes up garbled in some way. We can fix it by reloading theme after making newa frame.
#+begin_src emacs-lisp
(when (daemonp)
